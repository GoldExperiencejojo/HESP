...
#     #
#     #     return x
#     def forward(self, x: torch.Tensor, is_x0=None):
#         if is_x0:
#             x = self.conv1(x)  # shape = [*, width, grid, grid]
#             x0 = x
#             x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
...
#                                                                           device=x.device), x],
#                           dim=1)  # shape = [*, grid ** 2 + 1, width]
#             x = torch.cat([x, torch.zeros(x.shape[0], self.positional_embedding.size(0) - x.size(1), x.shape[-1],
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, x0=None):
        return self.visual(image.type(self.dtype), x0)
    ...
    def encode_text(self, text, prompts=None, coop=False):
        if prompts is not None:###text prompt
            ...
            x = x + (self.positional_embedding.to(x.device) * self.mask1.to(x.device)).type(self.dtype).to(x.device) + (
                        self.positional_embedding_res.to(x.device) * self.mask2.to(x.device)).type(self.dtype).to(
                x.device)
            x = x.permute(1, 0, 2)  # NLD -> LND
            x = self.transformer(x)
            x = x.permute(1, 0, 2)  # LND -> NLD
