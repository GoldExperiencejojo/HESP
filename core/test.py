...
def test(prompt, normalization, criterion, testloader, outloader, epoch=None, **options):
    ...
    _pred_k_a, _pred_u_a, _labels = [], [], [] # 负表示的预测结果
    _pred_k_c, _pred_u_c = [], [] # 遵循clip的预测结果
    _pred_k_e, _pred_u_e = [], [] # 平均后总体类别预测
    ...
    with torch.no_grad():
        ...
        for frames, labels, label_text, video_name in testloader:
            ...
            with torch.set_grad_enabled(False):
                ...
                logits, _ = criterion(visual_embedding, y, my_center) # 基于负表示的预测PNE，i

                predictions_a = logits.data.max(1)[1] # 基于负表示的预测
                total += labels.size(0)
                correct_a += (predictions_a == labels.data).sum() # 基于负表示预测的准确率
                _pred_k_a.append(logits.data.cpu().numpy())
                _labels.append(labels.data.cpu().numpy())
                
                predictions_c = clip_logits.data.max(1)[1] # 遵循clip的预测结果
                correct_c += (predictions_c == labels.data).sum()
                _pred_k_c.append(clip_logits.data.cpu().numpy())
                
                votes = torch.stack([clip_logits, logits])
                e_logits = votes.mean(dim=0) # 遵循clip的预测与负表示预测取平均值得出总体类别预测
